...
class ShuffleNetV2():
    ...
    def rescale(self, input, out_shape=None, scale=2, name=None):
        if out_shape == None:
            print('None')
        ...
        out = fluid.layers.resize_bilinear(
            input=input, out_shape=out_shape, scale=scale, name=name)
        return out
    ...
    def net(self, input, class_dim=81, img_shape=[3, 300, 300]):
        ...
        conv1 = self.conv_bn_layer(input=input, filter_size=3, num_filters=input_channel, padding=1, stride=2,
                                   name='stage1_conv')
        ...
        temp = self.rescale(feature_maps[1], out_shape=[19, 19], scale=2, name=None)
        feature_maps[0] = fluid.layers.concat(input=[feature_maps[0], temp], axis=1)

        temp = self.rescale(feature_maps[2], out_shape=[10, 10], scale=2, name=None)
        feature_maps[1] = fluid.layers.concat(input=[feature_maps[1], temp], axis=1)

        temp = self.rescale(feature_maps[3], out_shape=[5, 5], scale=2, name=None)
        feature_maps[2] = fluid.layers.concat(input=[feature_maps[2], temp], axis=1)

        temp = self.rescale(feature_maps[4], out_shape=[3, 3], scale=2, name=None)
        feature_maps[3] = fluid.layers.concat(input=[feature_maps[3], temp], axis=1)

        temp = self.rescale(feature_maps[5], out_shape=[2, 2], scale=2, name=None)
